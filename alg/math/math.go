...
func power(a, b string) string {
	...
	for x.GT(n, newFormString("0")) {
		if x.EQ(x.Mod(n, newFormString("2")).ToInteger(), newFormString("1")) {
			// n的二进制位最低为1则需要计入贡献
			ans = x.Ride(ans, xContribute)
		}
		xContribute = x.Ride(xContribute, xContribute)
		n = x.Except(n, newFormString("2")).ToInteger()
	}
	return ans.String()
}
...
// Sqrt 平方根
func (h Handler) Sqrt(number string) string {
	var x, y BigNum
	i := x.Except(newFormString(number), newFormString("2"))
	n := newFormString(number)
	a := newFormString("0.00000001")
	two := newFormString("2")
	result := newFormString(power(i.String(), two.String()))
	for y.GT(x.Sub(result, n).Abs(), a) {
		i = x.Sub(i, y.Except(x.Sub(result, n), y.Ride(i, two)))
		result = newFormString(power(i.String(), two.String()))
	}
	return i.String()
}
